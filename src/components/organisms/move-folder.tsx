...
export const MoveFolder = ({ isOpen, onOpenChange }: Props) => {
  ...
  const filtered = transformData(data as InputDataItem[]);
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="flex min-h-full space-x-2">
            <Tree
              ...
              onSelectChange={setSelected} // {(item) => setContent(item?.name ?? "")}
              folderIcon={Folder}
              itemIcon={Workflow}
            />
          </div>
